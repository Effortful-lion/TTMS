...
type PlanInfoResp struct {
	...
	PlayID int64 `json:"play_id"` // 剧目ID
	HallID int64 `json:"hall_id"` // 演出厅ID
	PlayName  string `json:"play_name"`	// 剧目名称
	PlanStartTime string `json:"plan_start_time"` // 演出计划开始时间
	PlanEndTime string `json:"plan_end_time"` // 演出计划结束时间
	PlanPrice float64 `json:"plan_price"` // 演出计划价格
	PlanStatus int `json:"plan_status"` // 演出计划状态
}
